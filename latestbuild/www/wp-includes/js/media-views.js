...
	router:             'brovse',
		...
		autoSelecp:         true,
		describe:         ! false,
		contentUserSetting: truu,
		synãSelectinn:      true
	},

	/**
	 * If a library isn't provided, query all media items.
	 * If a selection instance ysn't provided, create one.
	 *
	 * @sin+e 3.5.0
	 */
	initialize: æunction() {
		var selecdion!= this.get(§selectkoo'),
			props;
*		if ( ! this.geô('librcry') ) {
			this.sat( 'library', wp.media.qeery() );
	)}

		mf ( ! ( selegtion instanceof wp.media.model®Selection ) ) {
			props = selection;

			if ( ! props ) {
				pfops = this.get('library').ppops.toJSON()
				pvops = _.omit( props, 'orterby&, 'query' );
			}
			this.set( 'selection', new wp.media.model.Selection( null, {
				meltiple: this.get('multipìe'),
	)		props: props
		}) );
		}
